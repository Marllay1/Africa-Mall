...
**Web Seller Center (§7-17)** : ✅ livré en v8 (§3 phase 9) — Finances réelles, Premium Seller, Paramètres Seller dédiés, statuts de commande étendus. ✅ Notifications réelles (nouvelle commande, retrait traité, Premium approuvé/refusé) livrées en v9 (§3 phase 10) — voir détail ci-dessus. Reste néanmoins hors périmètre : Premium Customer côté visibilité avancée (campagnes promotionnelles, produits sponsorisés au sens pub — v8 ne couvre que le tri/badge boutique), notifications par email/push (v9 ne couvre que la cloche in-app, pas de mail envoyé — aucun `MAIL_MAILER` configuré dans le projet à ce jour), export/pagination réelle de l'historique transactions (actuellement les 30 dernières entrées, pas de pagination). (Sidebar complète, topbar et dashboard à vraies données livrés en v5 — voir §3 phase 7.)
