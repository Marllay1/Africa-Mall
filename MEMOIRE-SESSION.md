...
**v14 — 2026-09-25** — Revirement hébergement : Laravel Cloud → Vercel (commit `4953e89`). Laravel Cloud abandonné (payant, carte refusée) — l'utilisateur veut rester 100% gratuit. Bascule vers **Vercel** (déjà connecté au repo GitHub depuis le début, d'où les checks Vercel rouges sur toutes les PR jusqu'ici) via le runtime communautaire `vercel-php@0.9.0` :
- `vercel.json` + `api/index.php` (point d'entrée qui `require public/index.php`).
- `bootstrap/app.php` redirige `storage_path()` vers `/tmp` quand `VERCEL=1` (seul chemin inscriptible en serverless) — uniquement pour les vues Blade compilées, puisque session/cache/queue tournent déjà en driver `database`. Logs → `stderr` via env `LOG_CHANNEL=stderr`.
- **Reverb (v13) ne peut pas tourner sur Vercel** (serverless = pas de process persistant) ⇒ bascule vers **Pusher Cloud** (offre gratuite) en prod ; `resources/js/echo.js` choisit Pusher si `VITE_PUSHER_APP_KEY` est présent, sinon retombe sur Reverb (dev local, toujours fonctionnel). Aucun changement PHP (le connecteur `pusher` existait déjà dans `config/broadcasting.php`, publié par `reverb:install`).
- **Base de données** : Vercel n'héberge pas de Postgres persistant gratuit ⇒ **Supabase** (Postgres + stockage fichiers, choisi par l'utilisateur, couvre aussi le besoin de stockage photos/vocaux de la Phase 2 à venir).
- Vérifié en local uniquement (`VERCEL=1 php artisan tinker` : vues compilées confirmées dans `/tmp/storage/framework/views`) — **pas de déploiement Vercel réel testé** (nécessite les variables d'environnement Vercel : `APP_KEY`, `DB_*` Supabase, `PUSHER_*`, `LOG_CHANNEL=stderr` — aucune n'est encore configurée par l'utilisateur au moment d'écrire ceci).

**v13 — 2026-09-25** — Messagerie temps réel Phase 1 : infra Laravel Reverb (commit `dc98925`). Début d'un chantier plus large demandé par l'utilisateur (plan détaillé, hors ce fichier : messagerie enrichie type « Allyza » adaptée à la stack Laravel/Blade — texte/photo/vocal/stickers, réactions, réponses, édition/suppression, accusés de lecture, « écrit... », Admin invitable dans une conversation ; + accueil Customer restructuré ; + politiques Admin éditables (tarifs Premium, catégories). Appels audio/vidéo WebRTC explicitement **reportés** (décision utilisateur, nécessite un serveur TURN à financer). Cette v13 ne couvre que la Phase 1 : remplace le sondage (4s) par du WebSocket réel (`laravel/reverb`, canal privé `conversation.{id}`, event `MessageSent` en `ShouldBroadcastNow` — pas de queue worker en prod), composant Alpine `chatThread` (partagé Customer/Seller) mis à jour pour écouter le canal en temps réel avec le sondage en simple filet de sécurité (20s). Ajoute aussi la table `conversation_participants` (non branchée encore) en socle pour la Phase 4 (Admin invité). Vérifié en direct (deux contextes navigateur headless, message livré <1,5s sans rechargement, aucun doublon).

**Changement d'hébergement — historique** : l'essai gratuit Railway du client se termine sous 5 jours. Décision initiale : Laravel Cloud (voir tentative ci-dessous) — **abandonnée** (payant, carte de paiement de l'utilisateur refusée) au profit de **Vercel** (gratuit), malgré l'incompatibilité de principe avec Reverb expliquée à l'utilisateur (tranchée en basculant le temps réel sur Pusher — voir v14 ci-dessus pour le détail complet de l'adaptation). *Tentative Laravel Cloud (abandonnée)* : CLI `laravel/cloud-cli` installée mais bloquée par la politique réseau du sandbox (`cloud.laravel.com` refusé) — n'a jamais abouti, aucune infra Laravel Cloud n'a été créée.

**v12 — 2026-09-25** — Statistiques Premium (commit `cf12562`) : dernier point de la roadmap Seller Center (§16, « statistiques Premium »). Nouvelle colonne `products.views_count`, incrémentée à chaque vue de la fiche produit côté Customer (`ProductController::show`), sauf pour le vendeur consultant sa propre fiche (évite qu'il gonfle artificiellement sa propre stat). `/seller/statistiques` affiche désormais, uniquement pour les boutiques Premium (`Shop::isPremium()`), les vues totales + un classement des 5 produits les plus vus avec leur nombre de ventes ; les vendeurs non-Premium voient à la place une carte d'incitation vers `/seller/premium`. Vérifié en direct (navigateur headless + curl) : compteur de vues qui s'incrémente, contenu exact affiché pour un vendeur Premium vs un vendeur standard.

Avec cette livraison, **tous les points identifiés dans la feuille de route Seller Center (§6) sont traités** — restent hors périmètre, notés explicitement : Premium Customer avancé (produits sponsorisés ciblés, campagnes promotionnelles avec réduction groupée — options non retenues cette session), fournisseur SMTP réel (v11 tourne en `MAIL_MAILER=log`, attend des identifiants), notifications push, export CSV/PDF de l'historique transactions.

**v11 — 2026-09-25** — Notifications Seller par email (commit `8311343`) : les 3 notifications livrées en v9 (nouvelle commande, retrait traité, Premium approuvé/refusé) passent désormais aussi par le canal `mail` (`via()` retourne `['database', 'mail']`), avec un `toMail()` dédié par notification. Aucun fournisseur SMTP réel choisi/configuré par l'utilisateur à ce stade — codé avec `MAIL_MAILER=log` en local (déjà le défaut du projet), vérifié en direct via `storage/logs/laravel.log` (sujet, destinataire, contenu et lien de chaque email corrects pour les 3 scénarios). **Reste à faire dès que l'utilisateur fournit des identifiants SMTP réels** (host/port/user/password via les variables d'environnement, jamais en clair dans le repo) : passer `MAIL_MAILER=smtp` (+ `MAIL_HOST`/`MAIL_PORT`/`MAIL_USERNAME`/`MAIL_PASSWORD`/`MAIL_SCHEME`) sur Railway pour un envoi réel — le code n'a pas besoin de changer, seule la config d'environnement diffère entre local (`log`) et production (`smtp`). Point important : le déclenchement de `NewOrderReceived` a été déplacé hors de la transaction DB de `PlaceOrder` et enveloppé d'un `try/catch` (comme les 2 sites Admin) pour qu'un échec d'envoi mail (réseau externe, donc intrinsèquement possible) n'annule jamais une commande/action déjà en base — pas de `ShouldQueue` car le projet ne fait tourner aucun `queue:work` en production (`railway.json`), la mise en queue enverrait les jobs dans un trou noir.

**v10 — 2026-09-25** — Pagination réelle de l'historique Finances (commit `e7daf18`) : `/seller/revenus` affichait au mieux les 30 dernières transactions sans navigation. `Seller\FinanceController::paginatedTransactions()` fusionne désormais `payments` et `withdrawals` en PHP (deux tables hétérogènes, pas de UNION SQL portable pgsql/mysql/sqlite) puis construit un vrai `LengthAwarePaginator` (15/page) — vue mise à jour avec `{{ $transactions->links() }}`, même composant Tailwind que `seller/orders`. Vérifié en direct avec 22 transactions de test (20 paiements + 2 retraits) : page 1 = 15 résultats, page 2 = 7, navigation fonctionnelle.

**v9 — 2026-09-25** — Notifications Seller réelles livrées (commit `9fed96c`) : la cloche du Seller Center (topbar + sidebar, jusqu'ici statique) affiche désormais un vrai compteur non-lu (même composant Alpine `unreadBadge` que Messages, sondage 15s) et une page `/seller/notifications` (Laravel notifications DB standard — `User` avait déjà `Notifiable`, seule la table manquait : `php artisan notifications:table`). Trois déclencheurs : nouvelle commande (`PlaceOrder`, une notif par boutique concernée), retrait traité (payé/refusé — nécessitait un écran Admin qui n'existait pas encore : `/admin/withdrawal-requests`, mirror exact de `seller-requests`/`premium-requests`), demande Premium approuvée/refusée. Vérifié en direct (3 scénarios via curl authentifié + capture navigateur) : badge, marquage lu à la consultation, contenu des 3 types de notification.

**v8 — 2026-09-25** — Seller Center §6 livré (commit `f8a3e9b`) : les 4 chantiers placeholder du Web Seller Center sont remplacés par de vraies fonctionnalités. **Commandes** : statuts étendus avec `preparation` et `litige`. **Paramètres Seller** (`/seller/parametres`) : formulaire dédié profil boutique (`Shop::name/description/logo_path`) + infos commerciales/paiement-retrait (`SellerProfile`). **Finances Seller** (`/seller/revenus`, `/seller/statistiques`) : revenus réels calculés depuis `orders`/`payments` (total/disponible/en attente/déjà retiré), historique transactions, demande de retrait vers le moyen de paiement configuré (nouvelle table `withdrawals`), statistiques 6 mois via Chart.js (CDN, comme Font Awesome). **Premium Seller** (`/seller/premium`) : 2 formules (Basique 5 000 FCFA/mois, Pro 15 000 FCFA/mois — tarifs et activation tranchés avec l'utilisateur en session), souscription Seller → validation manuelle Admin (nouvelle table `premium_subscriptions`, écran `/admin/premium-requests` miroir de `seller-requests`), pas de paiement réel branché (cohérent avec §6 — paiement réel toujours en fondation transverse à faire). Effet visible Customer : badge « Premium » sur boutique/produits (`Shop::isPremium()`) et tri prioritaire Pro > Basique > standard dans le marché + recherche (`Product::scopePremiumFirst()`, SQL portable pgsql/mysql/sqlite, pas de cron requis — expiration lue à la volée via `expires_at`). Vérifié en direct (navigateur headless local + curl authentifié) : demande retrait, approbation Premium admin, badge et réordonnancement du marché confirmés visuellement. Laravel Boost installé au passage (`AGENTS.md` régénéré, `.mcp.json`/`boost.json`/`.claude/skills` ajoutés). Aucun test automatisé ajouté : le projet n'a de tests que sur l'auth Breeze par défaut, décision cohérente avec l'existant plutôt que d'introduire des factories Order/Shop/SellerProfile isolément pour ce chantier.
...
| 9. Seller Center — Finances/Paramètres/Premium/Commandes | `f8a3e9b` | Voir détail v8 ci-dessus. Remplace les 4 dernières pages placeholder (`seller.simple`) du Seller Center par du réel. Nouvelles tables `withdrawals`, `premium_subscriptions`. Nouveau scope Eloquent `Product::scopePremiumFirst()` + `Shop::isPremium()/premiumTierLabel()`. Premier usage de Chart.js dans le projet (CDN, pas de dépendance npm ajoutée, cohérent avec Font Awesome) |
| 10. Notifications Seller | `9fed96c` | Voir détail v9 ci-dessus. Premier usage du système de notifications Laravel natif dans le projet (table `notifications`, `Notifiable` sur `User`). Ajoute l'écran Admin `/admin/withdrawal-requests` (manquant jusque-là — les retraits restaient bloqués en `pending` indéfiniment) |
| 11. Pagination Finances | `e7daf18` | Voir détail v10 ci-dessus. |
| 12. Notifications email | `8311343` | Voir détail v11 ci-dessus. |
| 13. Statistiques Premium | `cf12562` | Voir détail v12 ci-dessus. Clôture les points identifiés de la roadmap Seller Center §6. |
| 14. Messagerie temps réel — Phase 1 (Reverb) | `dc98925` | Voir détail v13 ci-dessus. Premier des ~6 chantiers du plan messagerie/accueil Customer/politiques Admin. |
| 15. Déploiement Vercel (runtime PHP + Pusher) | `4953e89` | Voir détail v14 ci-dessus. Remplace la cible d'hébergement Laravel Cloud (abandonnée) par Vercel, adapte le temps réel Reverb→Pusher. |

**Hors périmètre explicite (pas encore traité)** : pages Customer restantes (favoris, paiements, notifications, support, litiges, sécurité — dead links dans l'ancien `home.php`), promotions Seller, reste de l'Admin (litiges, commissions, gestion catégories/produits, modération boutiques/comptes, approbation paiements réels), chat vendeur, paiement réel (Mobile Money/carte — Premium et retraits restent donc à validation/traitement manuel Admin en attendant), endpoints API Admin/messagerie/notifications/favoris/avis/Premium, les interfaces AfricaMall/AfricaMall Business elles-mêmes (SPA Vue/React consommant l'API), SMS/OTP réel.
...
**Web Seller Center (§7-17)** : ✅ **entièrement traité** (v8 à v12, §3 phases 9-13) — Finances réelles + pagination, Premium Seller (souscription/paiement + statistiques Premium), Paramètres Seller dédiés, statuts de commande étendus, notifications (in-app + email). Voir détail de chaque livraison ci-dessus. Reste hors périmètre, par choix explicite ou en attente d'une décision utilisateur : Premium Customer côté visibilité avancée au-delà de ce qui est livré (produits sponsorisés ciblés type carrousel, campagnes promotionnelles avec réduction groupée — options proposées, non retenues cette session), fournisseur SMTP réel non configuré (v11 tourne en `MAIL_MAILER=log` local — attend que l'utilisateur fournisse des identifiants SMTP via les variables d'environnement Railway), notifications push (jamais demandées), export de l'historique transactions (CSV/PDF — jamais demandé explicitement, pas fait). (Sidebar complète, topbar et dashboard à vraies données livrés en v5 — voir §3 phase 7.)
...
**🔵 EN COURS — Chantier messagerie + accueil Customer + politiques Admin (démarré v13)** : plan détaillé négocié avec l'utilisateur, phases restantes à enchaîner dans cet ordre (voir v13 ci-dessus pour le détail Phase 1 déjà livrée) :
1. ✅ Infra Reverb + socle `conversation_participants` (v13, `dc98925`)
2. Messagerie enrichie : upload réel photo/vocal (disque local, `storage:link` déjà prévu dans `railway.json`/à porter vers Laravel Cloud), stickers/emojis (palette choco/cream/or, pas de nouvelles couleurs), réactions, réponses, édition/suppression, accusés de lecture par curseur, « écrit... »
3. Notifications Web Push (VAPID + Service Worker minimal, `laravel-notification-channels/webpush`) — décidé **maintenant** par l'utilisateur, pas encore fait
4. Admin invité dans une conversation existante (pas de boîte Admin séparée) — décision utilisateur : « il peut être invité dans une conversation seller-customer »
5. Accueil Customer : navbar (Home/Panier/Messagerie/Notifications), carrousel piloté par les vendeurs Premium eux-mêmes (pas juste les derniers produits), suggestions basées sur achats récents (masquées si insuffisant)
6. Politiques Admin : tarifs Premium éditables (remplace `PremiumSubscription::TIERS`), CRUD catégories produits

**Explicitement hors périmètre de ce chantier** (décision utilisateur) : appels audio/vidéo WebRTC — nécessite un serveur TURN (Cloudflare Realtime ou coturn) à choisir/financer, reporté à un chantier séparé.
...
**Fondations transverses nécessaires à plusieurs items ci-dessus** : paiement réel (§23 — Mobile Money/Orange Money/Moov Money/Wave/carte ; le choix de méthode est capturé depuis v5 mais rien n'est traité). En v8, retraits Seller et activation Premium contournent ce manque via validation manuelle Admin (mêmes decisions que l'approbation Seller existante) — à remplacer par un vrai flux de paiement/versement le jour où §23 est traité, sans changer le schéma (`withdrawals.status`/`premium_subscriptions.status` sont déjà prêts pour ça). Tables manquantes du §33 selon les features attaquées. (Messagerie temps réel §15/26 livrée en v4-v5 en version allégée — sondage, texte+images, layout deux volets ; **vrai WebSocket livré en v13** — voir le chantier en cours détaillé juste au-dessus pour ce qui reste : emojis/stickers, pièces jointes fichier/audio, accusés de lecture, Admin invité. Statut en ligne/hors ligne toujours pas prévu, à ajouter si demandé.)
