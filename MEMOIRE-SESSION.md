...
**Web Seller Center (§7-17)** : ✅ livré en v8 (§3 phase 9) — Finances réelles, Premium Seller, Paramètres Seller dédiés, statuts de commande étendus. ✅ Notifications réelles (nouvelle commande, retrait traité, Premium approuvé/refusé) livrées en v9 (§3 phase 10), canal email ajouté en v11 (§3 phase 12). ✅ Pagination réelle de l'historique Finances livrée en v10 (§3 phase 11) — voir détail ci-dessus pour chacune. Reste néanmoins hors périmètre : Premium Customer côté visibilité avancée (campagnes promotionnelles, produits sponsorisés au sens pub — v8 ne couvre que le tri/badge boutique), fournisseur SMTP réel non configuré (v11 tourne en `MAIL_MAILER=log` local — attend que l'utilisateur fournisse des identifiants SMTP via les variables d'environnement Railway, voir v11), notifications push (jamais demandées), export de l'historique transactions (CSV/PDF — jamais demandé explicitement, pas fait). (Sidebar complète, topbar et dashboard à vraies données livrés en v5 — voir §3 phase 7.)
