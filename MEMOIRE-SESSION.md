...
**Changement d'hébergement en cours** : l'essai gratuit Railway du client se termine sous 5 jours ⇒ migration décidée vers **Laravel Cloud** (plutôt que Vercel, incompatible avec Laravel + un process WebSocket persistant comme Reverb — Vercel est serverless et ne supporte plus PHP nativement). Le check Vercel qui échouait sur chaque commit de la PR #1 depuis le début de cette session est donc un faux départ à ignorer, pas à corriger. CLI `laravel/cloud-cli` installée mais **bloquée par la politique réseau du sandbox** (`cloud.laravel.com` refusé) — la bascule effective sur Laravel Cloud reste à faire dès que l'accès réseau de la session est élargi (ou à faire manuellement par l'utilisateur si l'urgence des 5 jours dépasse ce délai).
